...
class DataCollatorIOI:
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            ...
            input_ids = self.tokenizer(text, return_tensors="pt", max_length=self.max_length, padding='max_length',
                                       truncation=True).input_ids[0]
            corr_input_ids = \
            self.tokenizer(corr_text, return_tensors="pt", max_length=self.max_length, padding='max_length',
                           truncation=True).input_ids[0]
...
def main():
    ...
    for edge_sparsity in [0.94, 0.945, 0.95, 0.955, 0.96, 0.965, 0.97, 0.975, 0.98, 0.985, 0.99, 0.995, 1.0, 1.01, 1.02,
                          1.05, 1.1]:
        ...
        if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
            ...
            sys.argv = [
                ...
                "--output_dir",
                f"./data/runs/ioi-{TAG}-elr{ELR}-llr{LLR}-relr{RELR}-rllr{RLLR}-es{edge_sparsity}-ns{NODE_SPARSITY}-t{TOTAL}/",
