...
from src.custom_datasets import ColoredMNIST
...
def compute_total_effect(logit_bias, logit_antibias, label):
    if logit_bias.shape[-1] > 1:
        ...
        y = torch.softmax(logit_bias)[:, label]
    else:
...
def compute_indirect_effect(logit_bias_intervene, logit_antibias, label):
    if logit_bias_intervene.shape[-1] > 1:
        y_anti = torch.softmax(logit_antibias, dim=-1)[:, label]
        y = torch.softmax(logit_bias_intervene)[:, label]
    else:
        y_anti = torch.sigmoid(logit_antibias)
...
def main():
    ...
    vit_state_dict = torch.load(args.control_model_name_or_path)
    new_state_dict = {}
    for old_key, value in vit_state_dict.items():
        if "vit.encoder.layer" in old_key:
            new_key = old_key.replace("vit.encoder.layer", "vit.encoder")
            new_state_dict[new_key] = value
        else:
            new_state_dict[old_key] = value
    control_model = ViTHeadModel(
        config=vit_config,
    ...
    reverse_model = ViTHeadModel(
        config=vit_config,
        include_qkv=False,
        with_embedding_nodes=embedding,
        disable_linear_regularization_term=False,
    ).eval().to(args.device)
    reverse_model.load_state_dict(circuit_state_dict)
    ...
    info("[i] Loading data...")
    dataset = ColoredMNIST(root='/data/nvme1/yxpeng/PycharmProjects/pyvenv-experiments/vision-grokking/new_data', env='all_train', select_class=select_class,
                           transform=transforms.Compose([
                               transforms.ToTensor(),
                               transforms.Normalize((0.1307, 0.1307, 0.), (0.3081, 0.3081, 0.3081))
                           ]))
    if ablate_type == 'prediction' and select_class == 'all':
    ...
    for images, labels in bar:
        ...
        if ablate_type == 'color':
            ctft_images = images[:, [1,0,2]]
        elif ablate_type == 'prediction' and select_class == 'all':
            ctft_images_list = [dataset[random.choice(dataset_index[1 - label.item()])][0] for label in labels] # get the opposite class image
        ...
        for j in range(images.shape[0]):
            logit_target = logits[j].detach().cpu().item()
            ...
            chosen_class = (logits[j] > 0).int()

            accuracy += (chosen_class == labels[j]).int().detach().cpu().item()
            control_choice = (control_logits[j] > 0).int()
            reverse_choice = (reverse_logits[j] > 0).int()
            close_all_choice = (close_all_logits[j] > 0).int()
            control_ctft_choice = (control_ctft_logits[j] > 0).int()
